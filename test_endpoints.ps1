...
Write-Host "--- 1. Testing GET /api/movies (Port $Port) ---" -ForegroundColor Cyan
$movies = Invoke-RestMethod -Uri "http://localhost:$Port/api/movies"
...
$heatmap = Invoke-RestMethod -Uri "http://localhost:$Port/api/seats/heatmap?movieId=mov-dune2&showTime=18:30"
...
$opt = Invoke-RestMethod -Uri "http://localhost:$Port/api/seats/optimize" -Method Post -Body $optBody -ContentType "application/json"
...
$snack = Invoke-RestMethod -Uri "http://localhost:$Port/api/snacks/calculate" -Method Post -Body $snackBody -ContentType "application/json"
...
$split = Invoke-RestMethod -Uri "http://localhost:$Port/api/split-pay/create" -Method Post -Body $splitBody -ContentType "application/json"
...
$checkout = Invoke-RestMethod -Uri "http://localhost:$Port/api/bookings/checkout" -Method Post -Body $checkoutBody -ContentType "application/json"
...
$booking = Invoke-RestMethod -Uri "http://localhost:$Port/api/bookings/$($checkout.bookingRef)"
